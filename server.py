...
def august_list_workflows(
    ...
    """[August Platform] List all saved workflows available to the authenticated user.
    Workflows are reusable AI automation pipelines built on August's legal AI capabilities,
    such as contract review, due diligence checklists, or compliance monitoring.
    Each workflow has an ID, name, description, and configuration for triggering.

    Args:
        user_email: Optional filter to list workflows belonging to a specific user.
        limit: Maximum number of workflows to return.
        offset: Number of workflows to skip for pagination.

    Returns a list of workflow objects with IDs, names, descriptions, and run counts."""
    params: dict[str, Any] = {}
    if user_email:
    ...
    if offset is not None:
        params["offset"] = offset
    return json.dumps(_aug_get("/workflows/", params or None), indent=2)


@mcp.tool()
def august_get_workflow(
    workflow_id: str,
    project_id: Optional[str] = None
) -> str:
    """[August Platform] Retrieve the full details of a specific workflow by its ID.
    Returns the workflow definition including steps, triggers, input/output configuration,
    default data sources, scheduling settings, and event trigger rules.
    Use this before running or modifying a workflow to understand its current configuration.

    Args:
        workflow_id: Unique identifier of the workflow to retrieve.
        project_id: Optional project scope for access control.

    Returns full workflow object including steps, inputs, outputs, and trigger config."""
    params: dict[str, Any] = {}
    if project_id:
        params["project_id"] = project_id
    return json.dumps(_aug_get(f"/workflows/{workflow_id}", params or None), indent=2)
...
def august_run_workflow(
    ...
    """[August Platform] Manually trigger a workflow run (equivalent to pressing 'Run Now' in the UI).
    Runs the workflow with the specified data sources and inputs. This is the primary way
    to execute a legal AI workflow against a set of documents — for example, running a
    contract review workflow on a new set of agreements, or triggering a due diligence
    summary workflow for a deal.

    Args:
        workflow_id: ID of the workflow to run.
        trigger_type: How the workflow is triggered. One of: 'manual' (default), 'scheduled', 'event'.
        file_ids: Specific document IDs to process in this run.
        folder_ids: Folder IDs whose documents should be processed.
        tabular_review_ids: IDs of tabular reviews to use as input.
        playbook_ids: IDs of playbooks to use as input.
        email_ids: IDs of emails to use as input.
        inputs: Key-value pairs for workflow input fields (overrides defaults).
        project_id: Project scope for this run.
        run_context: Optional context string describing why this run was triggered.

    Returns a run object with run_id and initial status. Poll with august_get_workflow_run_detail."""
    body: dict[str, Any] = {"workflow_id": workflow_id, "trigger_type": trigger_type}
    if file_ids is not None:
        body["file_ids"] = file_ids
    if folder_ids is not None:
        body["folder_ids"] = folder_ids
    if tabular_review_ids is not None:
    ...
    return json.dumps(_aug_post("/workflows/run", body), indent=2)


@mcp.tool()
def august_trigger_workflow(
    workflow_id: str,
    ...
    """[August Platform] Trigger a workflow via the event-trigger API endpoint.
    Similar to august_run_workflow but intended for programmatic/event-driven invocations
    where you need to pass source event metadata (e.g., triggering from an upload event
    or external webhook). The event_type and event_context capture what caused the run.

    Args:
        workflow_id: ID of the workflow to trigger.
        trigger_type: One of: 'manual', 'scheduled', 'event' (default for this endpoint).
        file_ids: Document IDs to include in this run.
        folder_ids: Folder IDs to include.
        tabular_review_ids: Tabular review IDs to include.
        playbook_ids: Playbook IDs to include.
        email_ids: Email IDs to include.
        source_event_id: ID of the upstream event that caused this trigger (for 'event' type).
        source_trigger_id: ID of the event trigger rule that fired.
        event_type: One of: 'upload', 'email', 'webhook', 'calendar', 'schedule'.
        event_context: Arbitrary dict with event-specific data (e.g. doc_ids of just-uploaded files).
        inputs: Key-value pairs for workflow input fields.
        project_id: Project scope for this run.
        run_context: Optional context string.

    Returns a run object with run_id and initial status."""
    body: dict[str, Any] = {"workflow_id": workflow_id, "trigger_type": trigger_type}
    if file_ids is not None:
        body["file_ids"] = file_ids
    if folder_ids is not None:
    ...
    if source_trigger_id is not None:
        body["source_trigger_id"] = source_trigger_id
    ...
    if run_context is not None:
        body["run_context"] = run_context
    return json.dumps(_aug_post("/workflows/trigger", body), indent=2)


@mcp.tool()
def august_get_workflow_runs(
    workflow_id: str,
    limit: Optional[int] = None,
    offset: Optional[int] = None,
    project_id: Optional[str] = None
) -> str:
    """[August Platform] List all historical runs for a specific workflow.
    Returns run history with statuses, timestamps, trigger types, and input summaries.
    Use this to audit workflow execution, debug failures, or find specific run IDs
    to inspect with august_get_workflow_run_detail.

    Args:
        workflow_id: ID of the workflow whose runs to list.
        limit: Maximum number of runs to return.
        offset: Number of runs to skip for pagination.
        project_id: Optional project scope.

    Returns a paginated list of run objects with statuses and timestamps."""
    params: dict[str, Any] = {}
    if limit is not None:
        params["limit"] = limit
    if offset is not None:
    ...
    if project_id:
        params["project_id"] = project_id
    return json.dumps(_aug_get(f"/workflows/{workflow_id}/runs", params or None), indent=2)


@mcp.tool()
def august_get_workflow_run_detail(
    workflow_id: str,
    run_id: str,
    project_id: Optional[str] = None
) -> str:
    """[August Platform] Retrieve the full detail of a specific workflow run.
    Returns step-by-step execution results, intermediate outputs, error messages if failed,
    input data used, and any generated work products. Essential for reviewing AI outputs
    from automated legal workflows (e.g., reviewing extracted clauses, checking summaries).

    Args:
        workflow_id: ID of the workflow.
        run_id: ID of the specific run to retrieve.
        project_id: Optional project scope for access control.

    Returns detailed run object including step outputs, status per step, and final results."""
    params: dict[str, Any] = {}
    if project_id:
        params["project_id"] = project_id
    return json.dumps(_aug_get(f"/workflows/{workflow_id}/runs/{run_id}", params or None), indent=2)


@mcp.tool()
def august_schedule_workflow(
    workflow_id: str,
    cron_expression: str,
    timezone: str = "UTC",
    enable: bool = True,
    ...
    """[August Platform] Schedule a workflow to run automatically on a recurring cron schedule.
    Use this to automate recurring legal tasks — e.g., weekly contract review reports,
    daily compliance monitoring, or monthly matter summaries.

    Args:
        workflow_id: ID of the workflow to schedule.
        cron_expression: Standard cron expression defining the schedule.
            Examples: '0 9 * * 1' (every Monday at 9am), '0 0 1 * *' (first of every month).
        timezone: Timezone for the cron schedule (IANA timezone name). Default: 'UTC'.
            Examples: 'America/New_York', 'Europe/London', 'Asia/Singapore'.
        enable: Whether to immediately activate the schedule. Default: True.
        project_id: Optional project scope.

    Returns the updated workflow schedule configuration."""
    body: dict[str, Any] = {
    ...
    if project_id is not None:
        body["project_id"] = project_id
    return json.dumps(_aug_post("/workflows/schedule", body), indent=2)


@mcp.tool()
def august_update_workflow_defaults(
    workflow_id: str,
    file_ids: Optional[List[str]] = None,
    folder_ids: Optional[List[str]] = None,
    tabular_review_ids: Optional[List[str]] = None,
    ...
    """[August Platform] Update the default data sources and input values for a workflow.
    Default data sources are pre-populated when the workflow is triggered manually in the UI,
    reducing manual selection. Set defaults when a workflow is always run against the same
    folders or documents — for example, a contract review workflow that always uses the
    active contracts folder.

    Args:
        workflow_id: ID of the workflow to update defaults for.
        file_ids: Default document IDs to pre-populate on each run.
        folder_ids: Default folder IDs to pre-populate on each run.
        tabular_review_ids: Default tabular review IDs to pre-populate.
        playbook_ids: Default playbook IDs to pre-populate.
        input_defaults: Dict of input field keys to default values.
                        Example: {"jurisdiction": "New York", "review_type": "full"}
        project_id: Optional project scope.

    Returns the updated workflow object with new default configuration."""
    ...
    if input_defaults is not None:
        body["input_defaults"] = input_defaults
    if project_id is not None:
        body["project_id"] = project_id
    return json.dumps(_aug_post("/workflows/update-defaults", body), indent=2)


@mcp.tool()
def august_create_workflow_event_trigger(
    workflow_id: str,
    name: str,
    ...
    """[August Platform] Create an event trigger that automatically fires a workflow when events occur.
    Event triggers enable fully automated legal workflows — e.g., automatically reviewing
    every new contract uploaded to a folder, or triaging incoming client emails to the
    appropriate matter workflow.

    Event types:
      - 'upload': Fires when a document is uploaded to a monitored folder.
                  Requires data_source_id (the folder ID to watch).
      - 'email': Fires when an incoming email matches a natural-language rule.
                 Requires email_prompt (evaluated against each new email).
                 User must have a connected Outlook/email account.
      - 'webhook': Fires from an external HTTP webhook call.
      - 'calendar': Fires on calendar events.
      - 'schedule': Fires on a schedule (use august_schedule_workflow instead).

    Args:
        workflow_id: ID of the workflow to attach this trigger to.
        name: Display name for this trigger (for identification).
        event_type: Type of event that fires this trigger. Default: 'upload'.
        data_source_id: For 'upload' triggers: the folder/data-source ID to watch (required).
        email_prompt: For 'email' triggers: natural-language rule to evaluate against emails
                      (required). Example: "The email contains a new NDA or contract for review".
        rate_limit_per_minute: Maximum trigger fires per minute (default: 10, currently not enforced).
        project_id: Optional project scope.

    Returns the created event trigger object with its trigger_id."""
    body: dict[str, Any] = {
        "workflow_id": workflow_id,
        "name": name,
    ...
    if email_prompt is not None:
        body["email_prompt"] = email_prompt
    if project_id is not None:
        body["project_id"] = project_id
    return json.dumps(_aug_post("/workflows/event-triggers", body), indent=2)


@mcp.tool()
def august_list_workflow_event_triggers(
    workflow_id: str,
    project_id: Optional[str] = None,
    event_type: Optional[str] = None
) -> str:
    """[August Platform] List all event triggers configured for a specific workflow.
    Returns all trigger rules attached to the workflow, including their event types,
    status (enabled/disabled), data sources monitored, and firing rate limits.
    Use this to audit automation rules and understand when/how a workflow will fire.

    Args:
        workflow_id: ID of the workflow whose event triggers to list.
        project_id: Optional project scope for filtering.
        event_type: Optional filter by event type ('upload', 'email', 'webhook', etc.).

    Returns a list of event trigger objects with their IDs, configs, and statuses."""
    params: dict[str, Any] = {}
    if project_id:
        params["project_id"] = project_id
    if event_type:
        params["event_type"] = event_type
    return json.dumps(_aug_get(f"/workflows/{workflow_id}/event-triggers", params or None), indent=2)


@mcp.tool()
def august_update_workflow_event_trigger(
    trigger_id: str,
    name: Optional[str] = None,
    data_source_id: Optional[str] = None,
    email_prompt: Optional[str] = None,
    ...
    """[August Platform] Update an existing workflow event trigger's configuration.
    Use this to enable/disable triggers, change the monitored folder, update email-matching
    rules, or adjust firing rate limits without deleting and recreating the trigger.

    Args:
        trigger_id: Unique identifier of the event trigger to update.
        name: New display name for the trigger.
        data_source_id: New folder/data-source ID for 'upload' triggers.
        email_prompt: New natural-language rule for 'email' triggers.
        is_enabled: Set to True to enable or False to disable the trigger.
        rate_limit_per_minute: New rate limit (currently not enforced, reserved for future use).
        project_id: Optional project scope.

    Returns the updated event trigger object."""
    body: dict[str, Any] = {}
    if name is not None:
        body["name"] = name
    if data_source_id is not None:
    ...
    if project_id is not None:
        body["project_id"] = project_id
    return json.dumps(_aug_patch(f"/workflows/event-triggers/{trigger_id}", body), indent=2)


@mcp.tool()
def august_delete_workflow_event_trigger(
    trigger_id: str,
    project_id: Optional[str] = None
) -> str:
    """[August Platform] Delete an event trigger from a workflow, stopping it from firing automatically.
    After deletion, the workflow will no longer run in response to the trigger event
    (e.g., new file uploads to a folder, or incoming emails matching a rule).
    This action is irreversible — recreate the trigger with august_create_workflow_event_trigger if needed.

    Args:
        trigger_id: Unique identifier of the event trigger to delete.
        project_id: Optional project scope.

    Returns a deletion confirmation."""
    path = f"/workflows/event-triggers/{trigger_id}"
    if project_id:
...
@mcp.tool()
def august_resume_workflow_run(
    workflow_id: str,
    run_id: str,
    response: str,
    ...
    """[August Platform] Resume a workflow run that is paused waiting for human input.
    Some legal AI workflows include human-in-the-loop steps that pause execution
    and request attorney review before proceeding — for example, reviewing an AI-drafted
    contract clause, approving a risk assessment, or confirming a document classification.
    Use this to provide the human response and continue the workflow.

    Args:
        workflow_id: ID of the workflow with the paused run.
        run_id: ID of the specific run that is waiting for input.
        response: The human response/answer to provide to the workflow.
                  This could be 'approve', 'reject', free-text feedback, or structured data.
        input_key: Optional key identifying which specific input field this response is for,
                   when the workflow step expects named inputs.
        project_id: Optional project scope.

    Returns the updated run object with resumed status."""
    body: dict[str, Any] = {
        "workflow_id": workflow_id,
        "run_id": run_id,
        "response": response,
    ...
    if project_id is not None:
        body["project_id"] = project_id
    return json.dumps(_aug_post("/workflows/runs/resume", body), indent=2)


# ── Tabular Reviews (Extraction) ─────────────────────────────────────────────

@mcp.tool()
def august_extract_data(
    doc_ids: List[str],
    questions: List[dict],
    user_email: str,
    name: Optional[str] = None,
    ...
    project_id: Optional[str] = None,
    table_reference_doc_ids: Optional[List[str]] = None,
    template_id: Optional[str] = None
) -> str:
    """[August Platform] Start a tabular review (data extraction) across multiple legal documents.
    Tabular reviews are one of August's most powerful features — they run structured questions
    across a set of documents and return answers in a table/spreadsheet format.
    Ideal for due diligence checklists, contract comparison, lease abstraction, and
    regulatory compliance reviews across large document sets.

    Args:
        doc_ids: List of document IDs to include in the extraction (required).
        questions: List of question/column definitions. Each dict should have:
                   - 'name': column header
                   - 'prompt': the question to answer for each document
                   - 'type': optional answer type ('text', 'boolean', 'date', 'number')
                   Example: [{"name": "Governing Law", "prompt": "What is the governing law?"}]
        user_email: Email address of the user requesting the extraction.
        name: Optional display name for this tabular review.
        description: Optional description of the review's purpose.
        folders: Optional list of folder inputs (dicts with 'folder_id' and processing 'mode').
        batch_mode: If True, processes documents in batches for large sets. Default: False.
        project_id: Optional project scope for the review.
        table_reference_doc_ids: IDs of reference documents to provide context to every column.
        template_id: ID of a saved extraction template to use as the column definition.

    IMPORTANT: Extraction runs asynchronously. This returns a review_id.
    Poll with august_get_tabular_review_status, then retrieve data with august_get_tabular_review_entries.

    Returns a review object with review_id and initial processing status."""
    body: dict[str, Any] = {
        "doc_ids": doc_ids,
        "questions": questions,
        "user_email": user_email,
        "batch_mode": batch_mode,
        "table_reference_doc_ids": table_reference_doc_ids or [],
    }
    if name is not None:
        body["name"] = name
    if description is not None:
    ...
    return json.dumps(_aug_post("/extraction/extract", body), indent=2)


@mcp.tool()
def august_get_tabular_review(review_id: str) -> str:
    """[August Platform] Retrieve the metadata and configuration of a tabular review (data extraction job).
    Returns the review's name, description, column definitions, document list, status, and timestamps.
    Use this to understand what a tabular review is configured to extract before
    retrieving individual cell entries with august_get_tabular_review_entries.

    Args:
        review_id: Unique identifier of the tabular review to retrieve.

    Returns the full tabular review object with columns, docs, and status summary."""
    return json.dumps(_aug_get(f"/extraction/{review_id}"), indent=2)


@mcp.tool()
def august_get_tabular_review_entries(
    review_id: str,
    doc_ids: Optional[List[str]] = None,
    ...
    """[August Platform] Retrieve the extracted data entries from a completed tabular review.
    Returns the AI-extracted answers in cell format — one row per document, one cell per question.
    Supports filtering by specific documents or columns to retrieve subsets of the table.
    This is the primary way to read the results of a due diligence extraction or contract review.

    Args:
        review_id: Unique identifier of the tabular review.
        doc_ids: Optional list of document IDs to filter results to specific documents.
        column_names: Optional list of column names to filter results to specific questions.
        limit: Maximum number of entries to return (pagination).
        offset: Number of entries to skip (pagination).

    Returns a list of entry objects with document ID, column name, extracted value, and citations."""
    params: dict[str, Any] = {}
    if doc_ids:
    ...
    if offset is not None:
        params["offset"] = offset
    return json.dumps(_aug_get(f"/extraction/{review_id}/entries", params or None), indent=2)


@mcp.tool()
def august_get_tabular_review_excel(
    review_id: str,
    include_citations: Optional[bool] = None,
    excluded_columns: Optional[List[str]] = None
) -> str:
    """[August Platform] Get a download link for a tabular review exported as an Excel spreadsheet.
    Returns a pre-signed URL to download the extraction results in .xlsx format,
    ready to share with clients or import into document management systems.
    Useful for delivering due diligence checklists, lease abstracts, or contract comparisons.

    Args:
        review_id: Unique identifier of the tabular review to export.
        include_citations: If True, include source citations in the Excel output. Default: server-defined.
        excluded_columns: List of column names to omit from the exported Excel file.

    Returns a download URL object with a pre-signed link to the Excel file."""
    params: dict[str, Any] = {}
    if include_citations is not None:
        params["include_citations"] = include_citations
    if excluded_columns:
        params["excluded_columns"] = excluded_columns
    return json.dumps(_aug_get(f"/extraction/{review_id}/excel", params or None), indent=2)


@mcp.tool()
def august_get_tabular_review_status(review_id: str) -> str:
    """[August Platform] Poll the processing status of a running tabular review extraction.
    Tabular reviews process documents asynchronously — this endpoint returns progress indicators
    showing how many documents have been processed out of the total.
    Poll this until processing is complete before retrieving entries with august_get_tabular_review_entries.

    Status values typically include: 'pending', 'processing', 'completed', 'failed'.

    Args:
        review_id: Unique identifier of the tabular review to check.

    Returns status object with current state, progress counts, and any error messages."""
    return json.dumps(_aug_get(f"/extraction/status/{review_id}"), indent=2)


@mcp.tool()
def august_get_recent_reviews(
    user_email: str,
    limit: Optional[int] = None,
    offset: Optional[int] = None
) -> str:
    """[August Platform] List recent tabular reviews (extraction jobs) for a specific user.
    Returns a paginated history of all tabular reviews the user has created,
    with their names, statuses, document counts, column counts, and timestamps.
    Useful for finding existing extraction jobs to view results or share with colleagues.

    Args:
        user_email: Email address of the user whose recent reviews to list (required).
        limit: Maximum number of reviews to return.
        offset: Number of reviews to skip for pagination.

    Returns a list of tabular review objects sorted by recency."""
    params: dict[str, Any] = {"user_email": user_email}
    if limit is not None:
        params["limit"] = limit
    if offset is not None:
        params["offset"] = offset
    return json.dumps(_aug_get("/extraction/recent_reviews", params), indent=2)
